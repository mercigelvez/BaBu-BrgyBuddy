$(document).ready(function () {
  ...
  function validateUsername() {
    const newUsername = $('#usernameInput').val().toLowerCase()
    const currentUsername = '{{ current_user.username }}'.toLowerCase()
    const errorMessageContainer = $('.username-error-message')
  ...
  function validateEmail() {
    ...
    const emailErrorMessage = $('#emailInput').siblings('.invalid-feedback')
  
    if (!emailRegex.test(email)) {
      $('#emailInput').addClass('is-invalid')
      emailErrorMessage.text('Please enter a valid email address (no spaces, .c0m, or @test allowed).')
    } else {
